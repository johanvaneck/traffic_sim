...
speeds = {'car': 2.25, 'bus': 1.8, 'truck': 1.8,
          'bike': 2.5}  # average speeds of vehicles
...
# def scale(obj: dict, scale: float):
#     for key, val in obj.items():
#         for i in range(3):
#             obj[key][i] = int(val[i]*scale)
            ...
            if(direction == 'right'):
                self.stop = vehicles[direction][lane][self.index-1].stop \
                    - vehicles[direction][lane][self.index -
                                                1].image.get_rect().width - stoppingGap
            elif(direction == 'left'):
                self.stop = vehicles[direction][lane][self.index-1].stop \
                    + vehicles[direction][lane][self.index -
                                                1].image.get_rect().width + stoppingGap
                ...
                self.stop = vehicles[direction][lane][self.index-1].stop \
                    - vehicles[direction][lane][self.index -
                                                1].image.get_rect().height - stoppingGap
            elif(direction == 'up'):
                self.stop = vehicles[direction][lane][self.index-1].stop \
                    + vehicles[direction][lane][self.index -
                                                1].image.get_rect().height + stoppingGap
